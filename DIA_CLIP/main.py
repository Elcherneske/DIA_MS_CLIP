...
def init_arg_parser():
    ...
    parser.add_argument('--max_mz_range', default=2500, type=float)
    ...
    parser.add_argument('--model_chrom_ratio', default=0.5, type=float)
    #train parameters
...
def main():
    ...
    dia_clip = DIAClip(
        ...
        n_head=args.n_head,
        device=device,
        dim_feedforward=args.hidden_layer,
        dropout=args.dropout,
        peptide_vocab=args.peptide_vocab_path,
        modification_vocab=args.modification_vocab_path,
        peptide_model=args.model_peptide,
        spec_model_ratio=args.model_chrom_ratio
    ).to(device)
    print(f"Model Params: d_model:{args.d_model}, RT_dim:{args.RT_dim}, hidden_layer: {args.hidden_layer}, n_head: {args.n_head}, dropout: {args.dropout}, peptide model: {args.model_peptide}\n")
